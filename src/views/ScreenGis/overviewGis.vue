...
import gis_bg from "../../assets/images/gis-backgroung.png";
export default {
  data() {
    return {
      areaColor: [
  ...
  methods: {
    renderMap() {
      ...
      //   var self = this;
      /* var normalm = L.tileLayer.chinaProvider("GaoDe.Normal.PurplishBlue", {
        maxZoom: 18,
        minZoom: 5
      }); */
      var normalm = L.tileLayer(url);
    ...
    /* renderPoint(map) {
      ...
      if (self.isTest) {
        var stationIDs = [
          "56458cdd584ce60ca4df4604",
          "56458cdc584ce60ca4df45f6"
        ]; //测试站点
      } else {
        var stationIDs = [
      ...
      api._fqjl_dvtree().then(response => {
        $.each(response, function(i, ele) {
          if (!!ele.area && ele.lv == 1) {
            ...
            polygon.bindPopup(
              `<div class="site-box box-shadow-1" style="z-index:-1;">
                                    <h2 class="popupTitle">${ele.name}</h2>
                                    <div class="site-content"  style="width:450px;height:250px;">
                                        <div class="lineTitle"><span class="label">DMA分区数</span><div class="line">${i +
                                          3} <span class="util">个</span></div></div>
                                    </div>
                                    </div>`,
              {
                className: "bubble",

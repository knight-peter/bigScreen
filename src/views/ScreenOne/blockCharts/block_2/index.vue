<template>
  <div class="full-height inner-padding" style="justify-content: center;">
    <div
      class="chart-header flex-box"
    ...
    >
      ...
      <div class="m-chart-header__tag flex-grow-1">
        <div
          ...
          :style="{ 'background-color': colors.ZGSL }"
        ></div>
        <div class="chart-tag">总供水量</div>
        <div
          class="m-tag-icon"
          style="width:3%;height:60%"
          :style="{ 'background-color': colors.ZSSL }"
        ></div>
        <div class="chart-tag">总售水量</div>
        <div
          class="m-tag-icon"
          style="width:3%;height:60%"
          :style="{ 'background-color': colors.LSL }"
        ></div>
        <div class="chart-tag">漏损率</div>
        <div
          class="m-tag-icon"
          style="width:3%;height:60%"
          :style="{ 'background-color': colors.CXCL }"
        ></div>
    ...
    <div class="chart-body flex-box">
      <div class="quater-width full-height">
        <hd-chart auto-resize :options="allChartsOptions.ZGSL"></hd-chart>
      </div>
      <div class="quater-width full-height">
        <hd-chart auto-resize :options="allChartsOptions.ZSSL"></hd-chart>
      </div>
      <div class="quater-width full-height">
        <hd-chart auto-resize :options="allChartsOptions.LSL"></hd-chart>
      </div>
      <div class="quater-width full-height">
        <hd-chart auto-resize :options="allChartsOptions.CXCL"></hd-chart>
      </div>
    </div>
  </div>
</template>
  ...
  methods: {
    load_ZGSL(response) {
      ...
      for (let year in response) {
        ...
        tmp_chart_options.series[0].data.push(response[year].water_supply);
      }

<template>
  <div class="full-width-height flex-box"
       style="flex-flow: column;align-items: center">
    <h3 class="font-blue"
        style="font-size: 280%;letter-spacing:12px;margin-bottom: 16px">贵港被控分区计量系统</h3>
    <div class="flex-box m-block-group"
         style="">
      <div class="outter-block">
        <span class="block-box inner-name">供水量(万m³)</span>
        <span class="block-box inner-number">{{details.GSL.num}}</span>
        <span :class="{'font-red':details.GSL.compare.arrow=='▼','font-green':details.GSL.compare.arrow=='▲'}"
              class="inner-arrow"><span>{{details.GSL.compare.arrow}}</span><span>{{details.GSL.compare.percentage}}</span></span>
      </div>
      <div class="outter-block">
        <span class="block-box inner-name">售水量(万m³)</span>
        <span class="block-box inner-number">{{details.SSL.num}}</span>
        <span :class="{'font-red':details.SSL.compare.arrow=='▼','font-green':details.SSL.compare.arrow=='▲'}"
              class="inner-arrow"><span>{{details.SSL.compare.arrow}}</span><span>{{details.SSL.compare.percentage}}</span></span>
      </div>
      <div class="outter-block">
        <span class="block-box inner-name">免费水量(万m³)</span>
        <span class="block-box inner-number">{{details.MFSL.num}}</span>
        <span :class="{'font-red':details.MFSL.compare.arrow=='▼','font-green':details.MFSL.compare.arrow=='▲'}"
              class="inner-arrow"><span>{{details.MFSL.compare.arrow}}</span><span>{{details.MFSL.compare.percentage}}</span></span>
      </div>
      <div class="outter-block">
        <span class="block-box inner-name">漏损水量(万m³)</span>
        <span class="block-box inner-number">{{details.LSSL.num}}</span>
        <span :class="{'font-red':details.LSSL.compare.arrow=='▼','font-green':details.LSSL.compare.arrow=='▲'}"
              class="inner-arrow"><span>{{details.LSSL.compare.arrow}}</span><span>{{details.LSSL.compare.percentage}}</span></span>
      </div>
      <div class="outter-block">
        <span class="block-box inner-name">产销差(万m³)</span>
        <span class="block-box inner-number">{{details.CXC.num}}</span>
        <span :class="{'font-red':details.CXC.compare.arrow=='▼','font-green':details.CXC.compare.arrow=='▲'}"
              class="inner-arrow"><span>{{details.CXC.compare.arrow}}</span><span>{{details.CXC.compare.percentage}}</span></span>
      </div>
      <div class="outter-block">
        <span class="block-box inner-name">漏损率(万m³)</span>
        <span class="block-box inner-number">{{details.LSL.num}}</span>
        <span :class="{'font-red':details.LSL.compare.arrow=='▼','font-green':details.LSL.compare.arrow=='▲'}"
              class="inner-arrow"><span>{{details.LSL.compare.arrow}}</span><span>{{details.LSL.compare.percentage}}</span></span>
      </div>
